...
\item D response with a special ACK to the sender to it, the sender constructs a path by appending it in front of the path, and goes on sending this ACK to the sender, until S. 
\item In this case, S knows the message is sent successfully, S can now discard the message, and a valid path is gotten from the ACK.
\end{enumerate}
...
The correctness of the location information is very important, and the information is quite small, so we can add redundant ECC code to correct the information instead of resending.\\[1em]

To make the location information be received to the base as soon as possible, each node broadcast the location message until it receives ACK from all nodes within the loss probability limit.\\[1em]
...
\item the sender sends each of these messages to only one nodes within the loss probability 
...
\section*{2.3~~~~Detailed Handling of Extreme Circumstances}
...
\section*{2.3.2~~~~Burst Mode}
 ...
 location information 5min, accelerate the frequency of rebroadcast, but it is necessary, and this method is the best.
